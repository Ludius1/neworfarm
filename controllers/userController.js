...
const registerUser = async (req, res) => {
    try {
        ...
        if((password.trim()).length < 6) {
            return res.status(400).json({msg: 'Password should be at least 6 charcters'})
        }
        ...
        const newUser = await User.create({firstName, lastName, email: newEmail, userName, password:hashedPassword})
...
const loginUser = async (req, res, next) => {
    try {
        ...
        const token = jwt.sign({id, firstName}, process.env.JWT_SECRET, {expiresIn: '6h'})
        console.log("Login Suucesful")
        console.log(user.firstName)

        // return res.status(200).json({ msg: `Welcome, ${user.firstName}` });
        return res.status(200).json({token, msg: `Welcome, ${user.firstName}` });


    }catch(error) {
        console.log(error)
        return res.status(500).json({msg: 'User login failed', error: error.message})
    }
}
...
const logoutUser = async (req, res) => {
    try {
        ...
        console.log(error)
        return res.status(500).json({msg: 'Error logging user', error: error.message})
    }
}
...
const upDateUser = async (req, res, next) => {
    ...
    try {
        const {firstName, lastName, userName, email, currentPassword, newPassword, confirmNewPassword} = req.body 
        if(!firstName || !lastName || !userName || !email || !currentPassword || !newPassword || !confirmNewPassword) {
        ...
        const newInfo = await User.findByIdAndUpdate(req.user.id, {firstName, lastName, email, userName, password: hashnewPassword}, {new: true})
...
module.exports = {registerUser, loginUser, upDateUser, getUser,logoutUser, userloginStatus,  changeAvatar, editProfile}
